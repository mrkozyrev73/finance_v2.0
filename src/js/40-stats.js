...
const Stats = (() => {
  ...
  function render(opts) {
    ...
    const scopeLabel = scope.type === 'year'
      ? String(scope.year) + (selectedYear ? ' · по месяцам' : ' год')
      : keyLabel(scope.key);
    ...
    $('#structYearBars').classList.toggle('hbars--months', Boolean(selectedYear));
    $('#structYearBars').classList.toggle('hbars--cats', !selectedYear);

    if (yearStruct) {
      const monthValues = Array.from({ length: 12 }, (_, m) => monthTotal(mkey(structureYear, m)));
      const chartRows = selectedYear
        ? monthValues.map((amount, m) => ({ name: MONTHS_SHORT[m], amount, color: amount > 0 ? 'var(--accent)' : 'var(--surface-sunk)' }))
        : topSlices(data.rows, YEAR_BARS);
      ...
      structYearNodes.forEach((n, i) => {
        ...
        if (selectedYear) {
          ...
          n.fill.style.width = Math.max(3, (row.amount / catMax) * 100) + '%';
          n.fill.style.height = '100%';
        }
