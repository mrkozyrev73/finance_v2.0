...
const IncomeList = (() => {
  ...
  return { render, flash, clearFeedback, preview, hold, release, reset: () => { nodes.clear(); heldPositions.clear(); } };
})();

/** Отметить доход полученным или вернуть в ожидаемые — прямо из строки */
function toggleReceived(id) {
  const pending = toggleReceived.pending && toggleReceived.pending.get(id);
  if (pending) {
    clearTimeout(pending.timer);
    toggleReceived.pending.delete(id);
    toggleReceived.busy && toggleReceived.busy.delete(id);
    const rec = Store.byId('incomes', id);
    if (rec) Home.previewIncome(id, rec.status);
    Home.clearIncomeFeedback(id);
    haptic('light');
    return;
  }
  ...
  if (!toggleReceived.pending) toggleReceived.pending = new Map();
  const timer = setTimeout(() => {
    toggleReceived.pending.delete(id);
  ...
  toggleReceived.pending.set(id, { timer });
...
const Home = (() => {
  ...
  return {
    ...
    clearIncomeFeedback: IncomeList.clearFeedback,
