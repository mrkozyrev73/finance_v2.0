/* ============================================================
   ...
   втором». Поэтому:
   • серия быстрых правок схлопывается в один запрос;
...
const Sync = (() => {
  ...
  async function onSignedIn() {
    if (signInSyncTask) return signInSyncTask;
    lastCatchUpAt = Date.now();
    signInSyncTask = finishSignInSync();
    try { return await signInSyncTask; }
    finally { signInSyncTask = null; }
  }

  async function finishSignInSync() {
  ...
  // У объектов из JSONB может отличаться порядок ключей; сравниваем
  // каноническое представление, сохраняя порядок записей в массивах.
  ...
  /** Быстрые соседние изменения отправляются одним общим снимком состояния. */
  function schedulePush() {
    if (!client || !session) { pendingPush = true; return; }
    pendingPush = true;
    clearTimeout(pushTimer);
    pushTimer = setTimeout(() => { pushTimer = 0; push(); }, PUSH_WINDOW);
  }
  ...
  function catchUp(reason, force) {
    if (!client || !session || !navigator.onLine) return Promise.resolve();
    if (catchUpTask) return catchUpTask;
    const nowAt = Date.now();
    if (!force && nowAt - lastCatchUpAt < CATCH_UP_COOLDOWN) return Promise.resolve();
    lastCatchUpAt = nowAt;
    ...
    const task = pull()
      ...
      .finally(() => { if (catchUpTask === task) catchUpTask = null; });
    catchUpTask = task;
    return task;
  }
      ...
      build(body) {
        ...
        if (session) {
          ...
          body.appendChild(el('button', {
            ...
            onclick: () => { catchUp('Обновляем…', true); Toast.show('Проверяем изменения'); }
